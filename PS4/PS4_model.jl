...
# Solve retiree's problem with backward induction
# function retiree_vectorized(input::Input, output::Output; updating::Bool=false)
    @unpack γ, σ, a_length, a_grid, N, Jr, r, b, β = input
    ...
    # for j = N:-1:Jr
        # for i = 1:a_length # iterate over possible asset states
            vals = fill(-Inf, a_length)
            if j == N # last period, no real choice to make
                vals .= ur((1+r) * a_grid[i] + b, γ, σ)
            else
...
# Solve worker's problem with backward induction
function worker_vectorized(input::Input, output::Output; updating::Bool=false)
    @unpack γ, σ, a_length, a_grid, N, Jr, r, b, β, w, θ, eta, z, Π = input
...
function solve_model(input::Input, output::Output; tol::Float64=1e-3, maxiter::Int64=10000)
    ...
    while err > tol && counter < maxiter
        ...
        output.K_SS = 0.01 * K_next + 0.99 * output.K_SS
        output.L_SS = 0.01 * L_next + 0.99 * output.L_SS
